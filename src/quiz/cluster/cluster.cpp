...
void Proximity(KdTree* tree, const std::vector<std::vector<float>>& points, std::vector<int>& cluster, std::vector<bool>& flag, int i) {
	...
	if (!flag[i]) {
		...
		// Iterate  through each nearby point
  		for(int index : nearby) {
			// Recursion
			Proximity(tree, points, cluster, flag, index);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int i=0;i<points.size();i++) {
		...
		if(!flag[i]) {
			...
			// Build the cluster
			Proximity(tree, points, cluster, flag, i);
